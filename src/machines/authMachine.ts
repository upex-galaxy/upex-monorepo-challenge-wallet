	...
	},
	{
		services: {
			...
			updateProfile: async (ctx, event: any) => {
				const payload = omit('type', event);
				const resp = await httpClient.patch(`http://localhost:${backendPort}/users/${payload.id}`, payload);
				return resp.data;
			},
			...
			getCognitoUserProfile: /* istanbul ignore next */ (ctx, event: any) => {
				...
				// Set Access Token in Local Storage for API calls
				localStorage.setItem(process.env.REACT_APP_AUTH_TOKEN_NAME!, event.user.signInUserSession.accessToken.jwtToken);
