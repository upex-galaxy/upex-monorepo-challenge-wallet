...
export const transactionDetailMachine = dataMachine('transactionData').withConfig({
	services: {
		fetchData: async (ctx, event: any) => {
			...
			const resp = await httpClient.get(`http://localhost:${backendPort}/transactions/${transactionId}`);
			// @ts-ignore
			return { results: [resp.data.transaction] };
		},
		createData: async (ctx, event: any) => {
			const route = event.entity === 'LIKE' ? 'likes' : 'comments';
			const payload = flow(omit('type'), omit('entity'))(event);
			const resp = await httpClient.post(`http://localhost:${backendPort}/${route}/${payload.transactionId}`, payload);
			return resp.data;
		},
		updateData: async (ctx, event: any) => {
			...
			const resp = await httpClient.patch(`http://localhost:${backendPort}/transactions/${transactionId}`, payload);
			return resp.data;
		},
	},
});
