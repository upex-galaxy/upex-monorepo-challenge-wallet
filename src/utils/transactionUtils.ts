...
export const isRequestTransaction = (transaction: Transaction) => flow(get('requestStatus'), negate(isEmpty))(transaction);
...
/* istanbul ignore next */
export const getFakeAmount = (min = 1000, max = 50000) => parseInt(faker.finance.amount(min, max), 10);
...
export const getChargeAmount = (sender: User, transaction: Transaction) => Math.abs(payAppDifference(sender, transaction).getAmount());

export const getTransferAmount = curry((sender: User, transaction: Transaction) => Math.abs(payAppDifference(sender, transaction).getAmount()));

export const getPayAppCreditedAmount = (receiver: User, transaction: Transaction) => Math.abs(payAppAddition(receiver, transaction).getAmount());

export const hasSufficientFunds = (sender: User, transaction: Transaction) => payAppDifference(sender, transaction).isPositive();

/* istanbul ignore next */
export const receiverIsCurrentUser = (currentUser: User, transaction: Transaction) => isEqual(get('id', currentUser), get('receiverId', transaction));

export const formatFullName = (user: User) => flow(pick(['firstName', 'lastName']), values, join(' '))(user);

export const isCommentNotification = (notification: NotificationType): notification is CommentNotification => has('commentId')(notification);

export const isLikeNotification = (notification: NotificationType): notification is LikeNotification => has('likeId')(notification);
...
export const getDateQueryFields = (query: TransactionDateRangePayload) => pick(['dateRangeStart', 'dateRangeEnd'], query);
...
/* istanbul ignore next */
export const hasPaginationQueryFields = (query: TransactionQueryPayload | TransactionAmountRangePayload) => has('page', query) && has('limit', query);
